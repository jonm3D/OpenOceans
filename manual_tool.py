...
def query_sliderule():
    ...
    if gdf.empty:
        w_status_box.text = 'No tracks found! Check bounding box, time window, or internet connection and try again.'
        return

    else:
        w_status_box.text = 'Found tracks, loading data...'
    ...
    data = dict(x=[], y=[], lon=[], lat=[], rgt=[],
                cycle=[], pair=[], track=[], date=[])
        ...
        for cycle_ in gdf.cycle.unique():
            for pair_ in gdf.pair.unique():
                for track_ in gdf.track.unique():
                    ...
                    gdf_ = reduce_gdf(
                        gdf, RGT=rgt_, track=track_, cycle=cycle_, pair=pair_)

                    if gdf_.shape[0] > 0:
                        # downsample...
                        if gdf_.shape[0] > 10:
                            # get evenly spaced indices of about 1,000 points
                            gdf_ = gdf_.iloc[np.floor(np.linspace(
                                0, gdf_.shape[0]-1, np.int64(10))), :]
    ...
    unique_date_list = [x.strftime('%Y-%m-%d')
                        for x in np.unique(data['date'])]
...
def update_track_details(attr, old, new):
    date_selected = datetime.datetime.strptime(
        w_sr_dates.value, '%Y-%m-%d').date()
    print(date_selected)
...
trackline = fig_imagery.line(
    x='x_wm', y='y_wm', source=src_gt, color='red', line_width=1)
...
fig_primary.circle(x=wm_axis,
                   ...
                   size=0.5,
                   color='grey',
                   legend_label='Subsurface Noise (Class {})'.format(
                       no_bottom_class),
                   name='subsurface',
                   selection_color='red',
...
sliderule_layout = column(fig_bbox,
                          row(w_date_picker_start, w_date_picker_end,
                              w_release_select, w_surftype_select),
                          w_conf_select,
                          w_query_button)
